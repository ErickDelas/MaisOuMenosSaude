...
import cargos_empresa.CargosEmpresa;
...
public class Usuario {
	...
	public static void inciarUsuarios() {
		...
		gerenteCF = new Gerente("g1", "g1", "g1", "g1", "g1", "g1", 10000, LocalDate.now(), 100,
				new Credenciais("g1", "g1"));
		gerenteCF.setCidade(Cidades.CORONEL_FABRICIANO);
		BDados.getGerentes().add(gerenteCF);
		gerenteI = new Gerente("g2", "g2", "g2", "g2", "g2", "g2", 20000, LocalDate.now(), 200,
				new Credenciais("g2", "g2"));
		gerenteI.setCidade(Cidades.IPATINGA);
		BDados.getGerentes().add(gerenteI);
		gerenteT = new Gerente("g3", "g3", "g3", "g3", "g3", "g3", 30000, LocalDate.now(), 300,
				new Credenciais("g3", "g3"));
		gerenteT.setCidade(Cidades.TIMOTEO);
		BDados.getGerentes().add(gerenteT);

		for (int i = 1; i <= 3; i++) {
			atendente = new Atendente("a" + i, "a" + i, "a" + i, "a" + i, "a" + i, "a" + i, 1000, LocalDate.now(), 100,
		...
		for (int i = 4; i <= 6; i++) {
			atendente = new Atendente("a" + i, "a" + i, "a" + i, "a" + i, "a" + i, "a" + i, 4000, LocalDate.now(), 400,
					new Credenciais("a" + i, "a" + i));
		...
		for (int i = 7; i <= 9; i++) {
			atendente = new Atendente("a" + i, "a" + i, "a" + i, "a" + i, "a" + i, "a" + i, 7000, LocalDate.now(), 700,
					new Credenciais("a" + i, "a" + i));
		...
		for (int i = 1; i <= 3; i++) {
			vendedor = new Vendedor("v" + i, "v" + i, "v" + i, "v" + i, "v" + i, "v" + i, 1000, LocalDate.now(), 100,
		...
		for (int i = 4; i <= 6; i++) {
			vendedor = new Vendedor("v" + i, "v" + i, "v" + i, "v" + i, "v" + i, "v" + i, 4000, LocalDate.now(), 400,
					new Credenciais("v" + i, "v" + i));
		...
		for (int i = 7; i <= 9; i++) {
			vendedor = new Vendedor("v" + i, "v" + i, "v" + i, "v" + i, "v" + i, "v" + i, 7000, LocalDate.now(), 700,
					new Credenciais("v" + i, "v" + i));
		...
		p.getEspecialidadesPlanos().add(new Especialidades("Cardiologista", 10.0));
		...
		if (retorno == CargosEmpresa.ADMINISTRADOR) {
			...
			do {
				for (Gerente g : BDados.getGerentes()) {
					if (usuario.equals(g.getCredenciaisGerente().getUsuario())
							&& senha.equals(g.getCredenciaisGerente().getSenha())) {
						Menu.acoesGerente(g);
						opcao = sc.nextInt();
			...
			do {
				for (Vendedor v : BDados.getVendedores()) {
					if (usuario.equals(v.getCredenciaisVendedor().getUsuario())
							&& senha.equals(v.getCredenciaisVendedor().getSenha())) {
			...
			do {
				for (Atendente a : BDados.getAtendentes()) {
					if (usuario.equals(a.getCredenciaisAtendente().getUsuario())
							&& senha.equals(a.getCredenciaisAtendente().getSenha())) {
	...
	public static CargosEmpresa verificarCredenciais(String usuario, String senha) {
		for (Adm administrador : BDados.getAdm()) {
			if (usuario.equals(administrador.getCredenciaisAdm().getUsuario())
					&& senha.equals(administrador.getCredenciaisAdm().getSenha())) {
